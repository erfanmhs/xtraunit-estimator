...
export const GUIDE: Record<GuideKey, GuideEntry> = {
  ...
  takeoff: {
    ...
    steps: [
      ...
      { do: "Each tool gets its own layer, named “Layer 1”, “Layer 2”… as you switch — start measuring straight away. Tap the layer chip to give it a real name (“Exterior walls”) or to continue an earlier layer of the same kind.", note: "A layer holds one kind of measurement, so areas and counts never mix. Every run you draw goes into the layer on the chip — tap it for “＋ New layer” (a second layer of the same kind: east walls, then west walls), to continue an earlier layer, or to stop drawing. Hold a run on the sheet to edit, move or delete it." },
